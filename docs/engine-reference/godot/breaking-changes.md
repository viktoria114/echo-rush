...
### AnimationPlayer — StringName type shift (4.6)

`AnimationPlayer` properties and methods now use `StringName` instead of `String`:
- `assigned_animation`, `autoplay`, `current_animation` → now `StringName`
- `get_queue()` return type → `StringName[]` (was `PackedStringArray`)
- Signal `current_animation_changed` parameter → `StringName`

**Fix:** Use `&"animation_name"` StringName literals. Plain strings still work but type annotations will mismatch.

### Networking — StreamPeer refactor (4.6)

Methods moved from `StreamPeerTCP` to base class `StreamPeerSocket`:
`disconnect_from_host()`, `get_status()`, `poll()`

Methods moved from `TCPServer` to base class `SocketServer`:
`is_connection_available()`, `is_listening()`, `stop()`

**Echo Rush impact:** `EchoAPI.gd` uses `HTTPRequest`, not raw TCP — no direct impact.

### Navigation — AStar2D behavior (4.6)

`AStar2D.get_point_path()` and related methods now return an empty path when `from_id` is disabled/solid (was undefined behavior before).
...
| TileMapLayer | Physics chunking default changed | `get_coords_for_body_rid()` returns different values. Set `physics_quadrant_size = 1` to restore 4.4 behavior |
| UI | `RichTextLabel.add_image()` signature | `size_in_percent` split into `width_in_percent` + `height_in_percent` |
