...
if [ -d "src/core" ] || [ -d "src/engine" ]; then
  if [ ! -d "docs/architecture" ]; then
    ...
    if [ "$ADR_COUNT" -lt 3 ]; then
      ...
      echo "    Suggested action: /reverse-document architecture scripts/core/[system]"
    fi
  fi
fi
    ...
    while IFS= read -r system_dir; do
      ...
      if [ "$file_count" -ge 5 ]; then
        ...
        if [ ! -f "$design_doc_1" ] && [ ! -f "$design_doc_2" ]; then
          echo "⚠️  GAP: Gameplay system 'scenes/$system_name/' ($file_count files) has no design doc"
          echo "    Expected: design/gdd/${system_name}-system.md or design/gdd/${system_name}.md"
          echo "    Suggested action: /reverse-document design scenes/$system_name"
        fi
      fi
